...
export default function EmployeeDashboard() {
  ...
  const refresh = async () => {
    setDataLoading(true);
    try {
      const [today, hist, lvs, anns, allEmps] = await Promise.all([
          ...
          <>
            ...
            {tab === 'history' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-border/30 bg-card/50 backdrop-blur-sm overflow-hidden">
                <div className="p-6 border-b border-border/30"><h3 className="font-semibold">Attendance History — Last 30 Days</h3></div>
                {history.length === 0 ? <div className="p-12 text-center text-foreground/40">No attendance records found.</div> : (
            ...
            {tab === 'leave' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                ...
                <div className="rounded-3xl border border-border/30 bg-card/50 backdrop-blur-sm overflow-hidden">
                  <div className="p-6 border-b border-border/30"><h3 className="font-semibold">My Leave Requests</h3></div>
                  {leaves.length === 0 ? <div className="p-12 text-center text-foreground/40">No leave requests found.</div> : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="border-b border-border/30 bg-background/30">
                          <tr>{['Type', 'From', 'To', 'Reason', 'Status'].map((h) => (
                            <th key={h} className="text-left px-6 py-3 text-xs uppercase tracking-wider text-foreground/40 font-semibold">{h}</th>
                          ))}</tr>
                        </thead>
                        <tbody>
            ...
            {tab === 'announcements' && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                {announcements.length === 0 ? <div className="p-12 text-center text-foreground/40">No announcements yet.</div> :
                  announcements.map((a) => (
            ...
            {tab === 'profile' && empData && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="rounded-3xl border border-border/30 bg-card/50 backdrop-blur-sm p-6 space-y-4">
                  ...
                  {[['Employee ID', empData.id], ['Department', empData.department], ['Designation', empData.designation], ['Email', empData.email], ['Phone', empData.phone]].map(([label, val]) => (
                  ...
                  <form onSubmit={handlePasswordChange} className="flex flex-col gap-4">
                    {[['Current Password', 'oldPass'], ['New Password', 'newPass'], ['Confirm New Password', 'confirmPass']].map(([label, field]) => (
                      <div key={field} className="flex flex-col gap-1.5">
                        ...
                        <input type="password" value={profileForm[field]} onChange={(e) => setProfileForm({ ...profileForm, [field]: e.target.value })}
                          className="bg-background/50 border border-border/50 rounded-xl px-4 py-3 outline-none focus:border-accent text-sm" placeholder="••••••••" required />
                      </div>
                    ))}
