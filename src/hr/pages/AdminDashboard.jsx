...
import {
  Users, Clock, CalendarOff, AlertCircle, Plus, Edit2, CheckCircle2, XCircle,
  Download, Bell, ChevronDown, ChevronUp, Search, X,
} from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const refresh = () => {
    ...
    setAttendance(getAttendance().sort((a, b) => new Date(b.date) - new Date(a.date)));
    setLeaves(getLeaves().sort((a, b) => new Date(b.appliedAt) - new Date(a.appliedAt)));
  ...
  const toggleStatus = (id, current) => {
    updateEmployee(id, { status: current === 'active' ? 'inactive' : 'active' });
    refresh();
    showToast(`Employee ${current === 'active' ? 'deactivated' : 'activated'}`);
  };

  const handleLeave = (id, status) => {
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="border-b border-border/30 bg-background/30">
                    <tr>
                      {['Employee', 'Type', 'From', 'To', 'Status', 'Action'].map((h) => (
                  ...
                  <tbody>
                    {leaves.slice(0, 5).map((l) => (
                      <tr key={l.id} className="border-b border-border/20 hover:bg-background/20">
                        ...
                        <td className="px-5 py-3">
                          {l.status === 'pending' && (
                            <div className="flex gap-2">
                              <button onClick={() => handleLeave(l.id, 'approved')} className="px-3 py-1 rounded-lg bg-green-500/20 text-green-400 text-xs hover:bg-green-500/30 transition-colors">Approve</button>
                              <button onClick={() => handleLeave(l.id, 'rejected')} className="px-3 py-1 rounded-lg bg-red-500/20 text-red-400 text-xs hover:bg-red-500/30 transition-colors">Reject</button>
                            </div>
                          )}
                        </td>
                      </tr>
            ...
            <div className="rounded-3xl border border-border/30 bg-card/50 backdrop-blur-sm overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="border-b border-border/30 bg-background/30">
                    <tr>
                      {['Name', 'ID', 'Department', 'Designation', 'Email', 'Status', 'Actions'].map((h) => (
                        <th key={h} className="text-left px-5 py-3 text-xs uppercase tracking-wider text-foreground/40 font-semibold">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {employees.map((emp) => (
                      <tr key={emp.id} className="border-b border-border/20 hover:bg-background/20 transition-colors">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex gap-2">
                            <button onClick={() => openEdit(emp)} className="p-1.5 rounded-lg bg-accent/10 text-accent hover:bg-accent/20 transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
              ...
              <div className="flex gap-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-4 py-2.5 rounded-xl bg-card/50 border border-border/40 outline-none focus:border-accent text-sm w-56"
                  />
                </div>
                <button onClick={exportAttendanceCSV}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-green-500/20 text-green-400 border border-green-500/30 text-sm font-medium hover:bg-green-500/30 transition-colors">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-border/30 bg-card/50 backdrop-blur-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {leaves.map((l) => (
                    <tr key={l.id} className="border-b border-border/20 hover:bg-background/20 transition-colors">
                      <td className="px-5 py-3 font-medium">{l.employeeName}</td>
                      <td className="px-5 py-3 capitalize">{l.type}</td>
                      <td className="px-5 py-3">{fmtDate(l.fromDate)}</td>
                      <td className="px-5 py-3">{fmtDate(l.toDate)}</td>
              ...
              <form onSubmit={handleAnnounce} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <textarea rows={4} value={annForm.body} onChange={(e) => setAnnForm({ ...annForm, body: e.target.value })}
                    ...
                    className="bg-background/50 border border-border/50 rounded-xl px-4 py-3 outline-none focus:border-accent text-sm resize-none" />
                </div>
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleEmpSubmit} className="grid grid-cols-2 gap-4">
                {[
                  ['Full Name', 'fullName', 'text', 'col-span-2'],
                  ['Employee ID', 'id', 'text', ''],
                  ['Department', 'department', 'text', ''],
                  ['Designation', 'designation', 'text', 'col-span-2'],
                  ['Email', 'email', 'email', ''],
                  ['Phone', 'phone', 'tel', ''],
                  ...
                  <div key={field} className={`flex flex-col gap-1.5 ${span}`}>
                    ...
                    <input
                      type={type} value={empForm[field]}
                      onChange={(e) => setEmpForm({ ...empForm, [field]: e.target.value })}
                      required disabled={field === 'id' && !!editingEmp}
                      placeholder={label}
